...
<div class="pageContent">
    <form>
        <div class="pageFormContent" layoutH="60">
            ...
            <p style="width: 99%">
                <label>操作员姓名：</label>
                <input type="text" name="realName" readonly="true" minlength="2" maxlength="45" size="30"
                       value="${pmsOperator.realName }"/>
            </p>
            <p style="width: 99%">
                <label>操作员登录名：</label>
                <input type="text" name="loginName" readonly="true" minlength="2" maxlength="45" size="30"
                       value="${pmsOperator.loginName }"/>
            </p>
            <p style="width: 99%">
                <label>创建时间：</label>
            ...
            <p style="width: 99%">
                <label>手机号码：</label>
                <input type="text" name="mobileNo" readonly="true" minlength="2" maxlength="45" size="30"
                       value="${pmsOperator.mobileNo }"/>
            </p>
            <p style="width: 99%">
                <label>状态：</label>
                <c:choose>
                    ...
                    <c:choose>
                        <c:when test="${pmsOperator.type eq 'ADMIN'}">
                            <label> <input type="checkbox"
                                           <c:if test="${pmsOperator.type eq 'ADMIN'}">disabled="disabled"</c:if>
                                           cssClass="required" name="selectRole" id="${item.id }">${item.roleName }
                            </label>
                        </c:when>
                        <c:when test="${pmsOperator.type eq 'USER'}">
                            <label> <input type="checkbox" disabled="disabled" cssClass="required" name="selectRole"
                                           id="${item.id }">${item.roleName }
                            </label>
                        </c:when>
                        <c:otherwise></c:otherwise>
                    </c:choose>
