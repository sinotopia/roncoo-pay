            ...
            <tr>
                ...
                <td>
                    <c:if test="${item.auditStatus=='NO'}">
                        <a title="设置支付方式" target="navTab"
                           href="${baseURL }/pay/way/list?payProductCode=${item.productCode}" style="color: blue;"
                           rel="zftdgl"> 设置支付方式</a>

                        <shiro:hasPermission name="pay:product:add">
                            <a title="确定要上架吗?" target="ajaxTodo" method="remove"
                               href="${baseURL }/pay/product/audit?productCode=${item.productCode}&auditStatus=YES"
                               style="color: blue;"> 上架</a>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="pay:product:delete">
                            <a title="确定要删除吗?" target="ajaxTodo" method="remove"
                               href="${baseURL }/pay/product/delete?productCode=${item.productCode}"
                               style="color: blue;"> 删除</a>
                        </shiro:hasPermission>
                    </c:if>
                    <c:if test="${item.auditStatus=='YES'}">
                        <a title="查看支付方式" target="navTab"
                           href="${baseURL }/pay/way/list?payProductCode=${item.productCode}" style="color: blue;"
                           rel="zftdgl"> 查看支付方式</a>
                        <shiro:hasPermission name="pay:product:add">
                            <a title="确定要下架吗?" target="ajaxTodo" method="remove"
                               href="${baseURL }/pay/product/audit?productCode=${item.productCode}&auditStatus=NO"
                               style="color: blue;"> 下架</a>
                        </shiro:hasPermission>
                    </c:if>
